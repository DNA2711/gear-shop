# 🛠️ GEARSHOP SYSTEM DOCUMENTATION

## 📋 TỔNG QUAN HỆ THỐNG

**GearShop** là một hệ thống thương mại điện tử chuyên về linh kiện máy tính, được xây dựng với **Next.js 15.3.2**, **TypeScript**, **MySQL**, và **Tailwind CSS**.

### 🎯 Các chức năng chính:
- **Quản lý sản phẩm**: Thương hiệu, danh mục, sản phẩm, hình ảnh, thông số kỹ thuật
- **Hệ thống đặt hàng**: Giỏ hàng, thanh toán VNPay, quản lý đơn hàng
- **PC Builder**: Xây dựng cấu hình máy tính, kiểm tra tương thích
- **Thông báo**: Hệ thống thông báo real-time cho người dùng và admin
- **Xác thực**: Đăng ký, đăng nhập, phân quyền JWT
- **Quản trị**: Dashboard admin với thống kê, quản lý
...
- **MVC Pattern**: Models (Types), Views (Components), Controllers (API Routes)
...
interface Brand {
  brand_id: number;
  brand_name: string;      // "ASUS", "MSI", "Intel"
  brand_code: string;      // "asus", "msi", "intel"
  logo_code?: string;      // Logo file reference
...
interface Category {
  category_id: number;
  category_name: string;   // "CPU", "VGA", "RAM"
  category_code: string;   // "cpu", "vga", "ram"
  parent_id?: number;      // Self-referencing for hierarchy
  is_active: boolean;
  created_at: string;
  updated_at: string;
}
...
interface Product {
  product_id: number;
  product_name: string;    // "Intel Core i7-13700K"
  product_code: string;    // "i7-13700k"
  brand_id?: number;       // FK to brands
...
interface User {
  ...
  createdAt: string;
  avatarCode?: string;     // Profile picture
}
```

#### **Order System (Hệ thống đặt hàng)**
```typescript
// File: src/types/order.ts
interface Order {
  id?: number;
  user_id: number;         // FK to users
  total_amount: number;    // Order total value
  status: "pending" | "paid" | "processing" | "shipped" | "delivered" | "cancelled";
  payment_method?: string; // "vnpay", "cod"
  payment_status?: string; // "paid", "failed", "pending"
...
GET    /api/brands/logos         # Get brand logos

GET    /api/categories           # List categories
GET    /api/categories?hierarchy=true  # Get category tree
...
### **Payment APIs:**
```
POST /api/vnpay/create-payment-url  # Generate VNPay payment URL
GET  /api/vnpay/callback           # VNPay payment callback
POST /api/vnpay/simulate-payment   # Simulate payment (Dev only)
```
...
```
User adds to cart → CartContext updates → User goes to checkout 
→ POST /api/orders (Order created with status 'pending') 
→ Redirect to VNPay payment → User completes payment 
→ VNPay callback updates order status to 'paid' 
→ Notifications created for user & admin → Order processing begins
```

**Key Implementation Files:**
- **Cart State**: `src/contexts/CartContext.tsx`
- **Order Creation**: `src/app/api/orders/route.ts`
- **Payment Processing**: `src/app/api/vnpay/callback/route.ts`
- **Notification Service**: `src/lib/notificationUtils.ts`

### **2. Notification System Flow:**

```
System event occurs → NotificationService.create() → Store in database 
→ NotificationContext polls API → Update UI badge/dropdown 
→ User clicks notification → Mark as read via API → Update local state
```

**Key Implementation Files:**
- **Service Layer**: `src/lib/notificationUtils.ts`
- **Context Provider**: `src/contexts/NotificationContext.tsx`
- **UI Component**: `src/components/ui/NotificationBell.tsx`

### **3. PC Builder Flow:**

```
User selects CPU → Filter compatible components → User adds more components 
→ Real-time compatibility check → Calculate total price & power 
→ Save build configuration → Add entire build to cart
```

**Key Implementation Files:**
- **PC Builder Context**: `src/contexts/PCBuilderContext.tsx`
- **Component Selector**: `src/components/pc-builder/ComponentSelector.tsx`
- **Compatibility API**: `src/app/api/pc-builder/compatibility/route.ts`
...
│   └── User Avatar/Login
├── Main Content
│   ├── Product Pages (Product Grid, Product Detail, Filters)
│   ├── Cart & Checkout (Cart Drawer, Checkout Form)
│   ├── PC Builder (Component Selector, Build Summary)
│   └── Admin Dashboard (Statistics, Data Tables, Forms)
└── Footer
```
...
#### **Product Display:**
- `src/components/ui/ProductCard.tsx` - Product grid item
- `src/components/pages/products/ProductDetail.tsx` - Detailed product view

#### **Shopping Cart:**
- `src/components/cart/CartDrawer.tsx` - Slide-out cart
- `src/components/cart/AddToCartButton.tsx` - Add to cart action

#### **Admin Interface:**
- `src/components/admin/DashboardStats.tsx` - Statistics dashboard
- `src/components/admin/ProductModal.tsx` - Product creation/editing
...
interface AuthContextType {
  ...
  login: (credentials) => Promise<{success: boolean}>;
  logout: () => void;
...
interface NotificationContextType {
  ...
  stats: NotificationStats;       // Total/unread counts
...
CREATE TABLE notifications (
  ...
  category ENUM('order_created', 'admin_new_order', 'system') NOT NULL,
...
### **JWT Implementation:**
- **Access Token**: Expires trong 1 giờ, lưu trong localStorage
- **Refresh Token**: Expires trong 7 ngày, để tự động refresh
- **Password Hashing**: Sử dụng bcryptjs với salt rounds = 10
- **Route Protection**: Middleware kiểm tra JWT cho protected routes

### **Authorization Levels:**
- **Guest**: Xem sản phẩm, đăng ký tài khoản
- **USER**: Mua hàng, xem đơn hàng, PC Builder, thông báo
- **ADMIN**: Quản lý sản phẩm, đơn hàng, thống kê, người dùng
...
- **Database Indexing**: Indexes trên foreign keys
- **Connection Pooling**: MySQL connection pool
- **Query Optimization**: Efficient joins và pagination
...
4. **TypeScript Strict Mode**: Stronger type safety

---

**📝 Document Version**: 1.0.0  
**📅 Last Updated**: January 2025  
**👥 Maintained By**: GearShop Development Team
...
interface ProductCardProps {
  ...
  layout?: "grid" | "list"; // Bố cục hiển thị
  className?: string;
}
  ...
  ({
    ...
    const handleAddToCart = async (e: React.MouseEvent) => {
      ...
      try {
        ...
        // Theo dõi sự kiện cho phân tích
    ...
    // Kiểm tra trạng thái wishlist khi mount
    useEffect(() => {
      if (user) {
        checkWishlistStatus();
      }
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // Logic hợp nhất các item trong giỏ hàng
...
```json
{
  ...
  "scripts": {
    ...
    // ===== TESTING =====
    "test": "jest", // Run tests
    "test:watch": "jest --watch", // Chạy tests ở chế độ watch
    "test:coverage": "jest --coverage", // Chạy tests với coverage
    "test:e2e": "playwright test", // Run E2E tests

    // ===== LINTING & FORMATTING =====
    "lint": "next lint", // Lint code
    "lint:fix": "next lint --fix", // Sửa lỗi linting
    "format": "prettier --write .", // Format code
    "type-check": "tsc --noEmit", // Kiểm tra kiểu TypeScript

    // ===== DEPLOYMENT =====
    "deploy:vercel": "vercel --prod", // Deploy lên Vercel
    "deploy:railway": "railway deploy", // Deploy lên Railway

    // ===== UTILITIES =====
    "clean": "rm -rf .next out", // Xóa các file build
  ...
  "dependencies": {
    ...
    "clsx": "^2.0.0", // Tiện ích class name
    "tailwind-merge": "^2.0.0", // Hợp nhất class Tailwind
...
const nextConfig: NextConfig = {
  ...
  swcMinify: true, // Dùng SWC để minify (nhanh hơn)
