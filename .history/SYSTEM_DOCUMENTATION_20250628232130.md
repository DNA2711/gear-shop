...
export const Header = ({ 
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      setSearchLoading(true);
      try {
        const response = await fetch(`/api/search/smart-suggestions?q=${encodeURIComponent(query)}`);
        if (response.ok) {
          const data = await response.json();
          if (data.success) {
    ...
    <header className={cn(
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between gap-4">
          {/* Logo */}
          <LoadingLink href="/" className="flex items-center space-x-2 flex-shrink-0">
            ...
            <span className="hidden sm:block text-xl font-bold text-gray-900">Gear Shop</span>
          </LoadingLink>

          {/* Main Navigation */}
          <nav className="hidden lg:flex items-center space-x-8">
            <LoadingLink href="/products" className="text-gray-700 hover:text-blue-600 font-medium transition-colors">
              Sản phẩm
            </LoadingLink>
            ...
            <LoadingLink href="/pc-builder" className="text-gray-700 hover:text-blue-600 font-medium transition-colors">
              Xây dựng PC
            </LoadingLink>
          </nav>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {items.map((item) => (
                  <CartItemComponent
                    ...
                    onRemove={() => handleRemoveItem(item.product_id, item.product_name)}
                  />
                ))}
              </div>
...
const CartItemComponent = ({ item, onQuantityChange, onRemove }: CartItemProps) => {
