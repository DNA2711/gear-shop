...
class QueryCache {
  ...
  async get<T>(key: string, queryFn: () => Promise<T>, ttl: number = 300000): Promise<T> {
...
export async function getProductsOptimized(filters: ProductFilter): Promise<ProductsResponse> {
...
export async function updateProductsStock(updates: Array<{productId: number, quantity: number}>) {
  const connection = await db.getConnection();
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-between">
        ...
        <span>
          {calculations.shippingFee === 0 ? 'Miễn phí' : `${calculations.shippingFee.toLocaleString('vi-VN')}₫`}
        </span>
      </div>
      <div className="flex justify-between font-bold text-lg">
        <span>Tổng cộng:</span>
        <span className="text-blue-600">{calculations.total.toLocaleString('vi-VN')}₫</span>
      </div>
    </div>
  );
});
...
class EmailService {
  ...
  async sendEmail(options: EmailOptions): Promise<boolean> {
    try {
      ...
      const text = template.text ? this.replaceTemplateVariables(template.text, options.data) : '';
      const subject = this.replaceTemplateVariables(template.subject, options.data);
  ...
  async sendPasswordReset(userEmail: string, resetToken: string, userName: string): Promise<boolean> {
  ...
  async sendLowStockAlert(products: Array<{
    product_name: string;
    ...
    const productsHtml = products.map(product => `
      <tr>
        ...
        <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: center;">
          <a href="${process.env.NEXTAUTH_URL}/admin/products/${product.product_id}/edit" 
        ...
        try {
          const success = await this.sendEmail({
            ...
            data: {
              content,
              subject,
              unsubscribeUrl: `${process.env.NEXTAUTH_URL}/unsubscribe?email=${encodeURIComponent(email)}`
            }
          });
