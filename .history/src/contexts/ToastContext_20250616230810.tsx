...
function GeneralToast({
  ...
  return (
    <div
      className={`fixed right-4 z-50 px-6 py-4 rounded-lg shadow-lg flex items-center gap-3 min-w-[300px] transform transition-all duration-300 ${getToastStyles()}`}
      style={{
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = (
    ...
    setToasts((prev) => {
      ...
      const currentToasts = Array.isArray(prev) ? prev : [];
