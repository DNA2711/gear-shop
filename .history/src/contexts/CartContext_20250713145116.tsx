...
export function CartProvider({ children }: CartProviderProps) {
  ...
  }, [cart, saveCartToStorage, isAuthenticated]);
  ...
  const updateCartItem = useCallback(
    async (product_id: number, quantity: number) => {
      if (!isAuthenticated) {
        showToast("Bạn cần đăng nhập để cập nhật giỏ hàng", "error");
        return;
      }
  ...
  const removeFromCart = useCallback(
    async (product_id: number) => {
      if (!isAuthenticated) {
        showToast("Bạn cần đăng nhập để xóa sản phẩm khỏi giỏ hàng", "error");
        return;
      }
  ...
  const clearCart = useCallback(async () => {
    if (!isAuthenticated) {
      showToast("Bạn cần đăng nhập để xóa giỏ hàng", "error");
      return;
    }
  ...
  }, [showToast, user?.id, isAuthenticated]);
  ...
  }, [cart.items, showToast, isAuthenticated]);
