...
export function CartProvider({ children }: CartProviderProps) {
  ...
  // Calculate totals
  const calculateTotals = (items: CartItem[]): { total_items: number; total_price: number } => {
  ...
  const addToCart = async (product_id: number, quantity: number = 1) => {
    ...
    try {
      ...
      setCart(prevCart => {
        const existingItemIndex = prevCart.items.findIndex(item => item.product_id === product_id);
  ...
  const refreshCart = async () => {
    ...
    try {
      ...
      if (updatedItems.length < cart.items.length) {
        showToast('Một số sản phẩm đã được cập nhật hoặc xóa khỏi giỏ hàng', 'warning');
      }
    } catch (error) {
