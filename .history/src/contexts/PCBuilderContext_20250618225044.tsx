...
function pcBuilderReducer(state: PCBuilderState, action: PCBuilderAction): PCBuilderState {
  switch (action.type) {
    case 'SET_BUILD_NAME':
      return {
        ...state,
    ...
    case 'ADD_COMPONENT': {
      ...
      if (category.multiple_allowed) {
        ...
        const existing = newComponents[type as keyof typeof newComponents] as PCComponent[] || [];
        newComponents = {
          ...newComponents,
    ...
    case 'REMOVE_COMPONENT': {
      ...
      if (category.multiple_allowed && typeof index === 'number') {
        ...
        const { [type]: removed, ...rest } = newComponents;
        newComponents = rest;
      }
...
export function PCBuilderProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    localStorage.setItem('pcbuilder-current-build', JSON.stringify(state.currentBuild));
  }, [state.currentBuild]);
  ...
  const getComponentCount = (type: string): number => {
    const component = state.currentBuild.components[type as keyof typeof state.currentBuild.components];
  ...
  const getComponentByType = (type: string): PCComponent | PCComponent[] | undefined => {
    return state.currentBuild.components[type as keyof typeof state.currentBuild.components];
  };
