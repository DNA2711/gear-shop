...
import { getCartStorageKey, clearAllCartDataFromStorage } from "@/utils/cartUtils";
...
export function CartProvider({ children }: CartProviderProps) {
  ...
  const getCartKey = useCallback(() => {
    if (user && user.id) {
      return `gear-shop-cart-user-${user.id}`;
    }
    return "gear-shop-cart-guest"; // Fallback for guest users
  }, [user]);
  ...
  // Clear cart data for all users (used on logout)
  ...
  useEffect(() => {
    if (cart.items.length > 0 || localStorage.getItem(getCartKey())) {
      saveCartToStorage();
    }
  }, [cart, saveCartToStorage, getCartKey]);

  // Clear cart when user logs out
  useEffect(() => {
    if (!isAuthenticated && user === null) {
      // User has logged out, clear cart
      clearAllCartData();
    }
