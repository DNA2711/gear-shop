import { PCComponent, PCBuild, CompatibilityStatus, CompatibilityWarning, CompatibilityError, PowerConsumption } from '@/types/pcbuilder';
...
export function extractSpecValue(component: PCComponent, specName: string): string {
...
export function calculatePowerConsumption(build: PCBuild): PowerConsumption[] {
  ...
  Object.entries(build.components).forEach(([type, component]) => {
    ...
    const category = COMPONENT_CATEGORIES[type as keyof typeof COMPONENT_CATEGORIES];
    if (!category) return;
...
export function getTotalEstimatedPower(build: PCBuild): number {
  ...
  return consumptions.reduce((total, consumption) => total + consumption.estimated_power, 0);
}

// Check socket compatibility between CPU and Mainboard
export function checkSocketCompatibility(cpu?: PCComponent, mainboard?: PCComponent): CompatibilityError | null {
...
export function checkMemoryCompatibility(ram?: PCComponent[], mainboard?: PCComponent): CompatibilityError | null {
...
export function checkPowerSupply(build: PCBuild): CompatibilityError | CompatibilityWarning | null {
...
export function checkCompatibility(build: PCBuild): CompatibilityStatus {
  ...
  const socketError = checkSocketCompatibility(build.components.cpu, build.components.mainboard);
  if (socketError) errors.push(socketError);
  
  // Check memory compatibility
  const memoryError = checkMemoryCompatibility(build.components.ram, build.components.mainboard);
  ...
  Object.entries(COMPONENT_CATEGORIES).forEach(([type, category]) => {
    if (category.required && !build.components[type as keyof typeof build.components]) {
      warnings.push({
        type: 'performance',
...
export function validateBuild(build: PCBuild): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  for (const componentType of requiredComponents) {
    if (!build.components[componentType as keyof typeof build.components]) {
      const category = COMPONENT_CATEGORIES[componentType as keyof typeof COMPONENT_CATEGORIES];
...
export function getComponentSuggestions(build: PCBuild, targetType: string): string[] {
