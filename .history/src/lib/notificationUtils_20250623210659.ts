...
import { CreateNotificationRequest, NotificationCategory, NotificationType } from "@/types/notification";
...
export class NotificationService {
  ...
  static async createOrderSuccessNotification(userId: number, orderData: any) {
    try {
      const notification: CreateNotificationRequest = {
        ...
        message: `Đơn hàng #${orderData.orderId} của bạn đã được đặt thành công với tổng giá trị ${formatPrice(orderData.totalAmount)}. Chúng tôi sẽ xử lý đơn hàng và thông báo cập nhật cho bạn sớm nhất.`,
  ...
  static async createNewOrderNotificationForAdmin(orderData: any) {
    try {
      ...
      const notifications = admins.map((admin: any) => ({
        ...
        title: "🛒 Đơn hàng mới!",
        message: `Có đơn hàng mới #${orderData.orderId} từ khách hàng ${orderData.customerName} với giá trị ${formatPrice(orderData.totalAmount)}. Vui lòng kiểm tra và xử lý.`,
  ...
  private static async createNotification(notification: CreateNotificationRequest) {
    try {
      const insertQuery = `
