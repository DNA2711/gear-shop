...
export class PasswordService {
  ...
  public validatePasswordStrength(password: string): {
    ...
    // Check for at least one letter (either upper or lower)
    if (!/[A-Za-z]/.test(password)) {
      errors.push("Mật khẩu phải chứa ít nhất một chữ cái");
    }

    // Check for number (optional - comment out to make it optional)
    if (!/\d/.test(password)) {
      errors.push("Mật khẩu phải chứa ít nhất một số");
    }

    // Optional: Check for special character (comment out to make it optional)
    // if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
