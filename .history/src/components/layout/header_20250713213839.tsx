...
export default function Header() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Chỉ reset selectedIndex khi không có suggestions
    ...
    // Nếu input rỗng, ẩn suggestions ngay lập tức
    if (value.length === 0) {
    ...
    // Delay 300ms thay vì 1s để responsive hơn
  ...
  const fetchSearchSuggestions = async (query: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Chỉ show suggestions nếu có kết quả và query match với searchTerm hiện tại
        if (suggestions.length > 0 && query === searchTerm) {
          setShowSuggestions(true);
        }
      }
    } catch (error) {
