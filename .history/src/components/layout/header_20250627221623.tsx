...
export default function Header() {
  ...
  const fetchSearchSuggestions = async (query: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/search/smart-suggestions?q=${encodeURIComponent(query)}&limit=8`);
      if (response.ok) {
                ...
                {showSuggestions && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 overflow-hidden z-50 max-h-[500px] overflow-y-auto animate-in slide-in-from-top-2 duration-200">
                    {searchSuggestions.length > 0 ? (
                      ...
                      /* Loading State */
                      <div className="p-8 text-center">
                        <div className="w-8 h-8 mx-auto mb-4 border-3 border-blue-200 border-t-blue-500 rounded-full animate-spin"></div>
