            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="h-36 p-4 rounded-xl bg-gray-800/50 border border-gray-700/50 flex flex-col"
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-10 h-10 bg-gray-700 rounded-lg animate-pulse flex-shrink-0"></div>
                    ...
                    <div className="flex-1 space-y-2 pt-1 pb-1">
            ...
            <div className="max-h-[65vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-2 p-4">
                {categories.map((category, index) => (
                  ...
                  >
                    ...
                    >
                      <div className="h-36 p-4 rounded-xl bg-gray-800/50 hover:bg-gray-700/70 transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-lg border border-gray-700/50 group-hover:border-gray-600/50 flex flex-col">
                        <div className="flex items-center gap-3 mb-2">
                          <div
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-semibold text-white group-hover:text-blue-300 transition-colors text-sm leading-tight">
                        ...
                        {/* Subcategories preview với chiều cao cố định */}
                        <div className="flex-1 overflow-hidden min-h-[3rem] pb-1 pt-1">
                          {category.children &&
                              ...
                              <div className="space-y-1">
                                {category.children.slice(0, 1).map((child) => (
                                  <div
                                    ...
                                    className="flex items-start gap-2 text-xs text-gray-400 hover:text-blue-400 transition-colors p-1 rounded cursor-pointer"
                                    onClick={(e) => {
                                      e.preventDefault();
                                      e.stopPropagation();
                                      handleCategoryClick(child.category_id);
                                    }}
                                  >
                                    <div className="w-1.5 h-1.5 bg-blue-400/60 rounded-full flex-shrink-0 mt-1"></div>
                                    <span className="leading-relaxed text-xs break-words">
                                      {child.category_name}
                                    </span>
                                  </div>
                                ))}
                                {category.children.length > 1 && (
                                  <div className="text-xs text-blue-400 font-medium flex items-start gap-1 p-1 mt-1">
                                    <ArrowRight className="w-3 h-3 flex-shrink-0 mt-0.5" />
                                    <span className="leading-relaxed break-words">
                                      +{category.children.length - 1} danh mục
                                      khác
                                    </span>
                                  </div>
                                )}
                        ...
                        <div className="absolute left-full top-0 ml-2 w-72 bg-gray-800/95 backdrop-blur-xl border border-gray-700/50 rounded-xl shadow-2xl p-3 z-50">
                          ...
                          <div className="grid grid-cols-1 gap-1">
                            {category.children.map((child) => (
                              <div
                                key={child.category_id}
                                className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-700/50 transition-colors text-sm text-gray-300 hover:text-white cursor-pointer"
                                onClick={() =>
                                  handleCategoryClick(child.category_id)
                                }
                              >
                                <div className="w-2 h-2 bg-blue-400/60 rounded-full flex-shrink-0"></div>
                                <span className="leading-relaxed break-words">
                                  {child.category_name}
                                </span>
                              </div>
              ...
              <div className="border-t border-gray-700/50 bg-gray-800/30 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className="flex items-center gap-2 text-blue-400 hover:text-blue-300 font-medium text-sm transition-colors group cursor-pointer"
                    onClick={() => {
                      setIsOpen(false);
