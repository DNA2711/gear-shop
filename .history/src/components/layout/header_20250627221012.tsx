...
export default function Header() {
  ...
  const [aiSearchEnabled, setAiSearchEnabled] = useState(true); // AI search toggle
  ...
  const fetchSearchSuggestions = async (query: string) => {
    ...
    try {
      ...
      // Parallel fetch: Product search + AI suggestions (if enabled)
      const promises = [
      ...
      // Add AI/Smart suggestions if enabled
      if (aiSearchEnabled) {
        promises.push(
          fetch("/api/search/smart-suggestions", {
            method: "POST",
      ...
      if (responses[1] && responses[1].ok) {
        const smartResult = await responses[1].json();
        if (smartResult.success) {
          setSmartSuggestions(smartResult.suggestions.slice(0, 6)); // Limit to 6
        }
      }
    } catch (error) {
  ...
  // Handle clicking on smart suggestion
          ...
          <div className="container flex items-center justify-between gap-5 h-24 relative">
            ...
            <div className="hidden md:flex flex-1 max-w-2xl mx-8">
              <div className="relative w-full">
                ...
                {showSuggestions && (searchSuggestions.length > 0 || smartSuggestions.length > 0) && (
                    ...
                    <div className="p-2">
                      ...
                      <div className="flex items-center justify-between p-2 border-b bg-gray-50">
                        <span className="text-xs text-gray-600">
                          🤖 AI Search {aiSearchEnabled ? 'Enabled' : 'Disabled'}
                        </span>
                        <button
                        ...
                        >
                          {aiSearchEnabled ? 'ON' : 'OFF'}
                        </button>
                      </div>

                      {/* Smart Suggestions (AI-powered) */}
                      {aiSearchEnabled && smartSuggestions.length > 0 && (
                        <div className="border-b pb-2 mb-2">
                          <div className="text-xs text-blue-600 font-medium p-2 flex items-center gap-1">
                            🧠 Gợi ý thông minh cho "{searchTerm}"
                          </div>
                          <div className="grid grid-cols-2 gap-1">
                            {smartSuggestions.map((suggestion, index) => (
                              <div
                                ...
                                className="px-3 py-2 hover:bg-blue-50 cursor-pointer transition-colors rounded text-sm text-gray-700 hover:text-blue-600 border border-blue-100 hover:border-blue-200"
                              >
