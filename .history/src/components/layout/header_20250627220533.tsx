...
export default function Header() {
  ...
  const fetchSearchSuggestions = async (query: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/products?search=${encodeURIComponent(query)}&limit=8&is_active=true`);
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
  ...
  const handleSearchChange = (value: string) => {
    ...
    // Set new timeout for debounced search (giảm từ 300ms xuống 100ms)
