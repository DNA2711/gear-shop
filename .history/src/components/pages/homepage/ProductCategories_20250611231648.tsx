    ...
    const fetchCategories = async () => {
      try {
        ...
        if (result.success && result.data) {
          // Filter parent categories (no parent_id) and active ones first
          const parentCategories = result.data.filter((cat: CategoryWithChildren) => 
            !cat.parent_id && cat.is_active === 1
          );
          
          console.log("Parent categories found:", parentCategories);

          // If not enough parent categories, include some child categories
          let allActiveCategories = result.data.filter((cat: CategoryWithChildren) => 
            cat.is_active === 1
          );
          ...
          const displayCategories: DisplayCategory[] = categoriesToDisplay.map((cat: CategoryWithChildren) => ({
            id: cat.category_id,
          ...
          if (displayCategories.length > 0) {
            ...
            const fallbackCategories: DisplayCategory[] = [
              { id: 1, name: "CPU", description: "Bộ vi xử lý", icon: <Cpu className="w-4 h-4 sm:w-5 sm:h-5" />, count: 125, color: "from-slate-400 to-slate-600", bgColor: "bg-slate-500/10" },
              { id: 2, name: "VGA", description: "Card đồ họa", icon: <Monitor className="w-4 h-4 sm:w-5 sm:h-5" />, count: 89, color: "from-blue-400 to-blue-600", bgColor: "bg-blue-500/10" },
              { id: 3, name: "RAM", description: "Bộ nhớ", icon: <HardDrive className="w-4 h-4 sm:w-5 sm:h-5" />, count: 167, color: "from-slate-500 to-blue-500", bgColor: "bg-slate-500/10" },
              { id: 4, name: "Storage", description: "Ổ cứng", icon: <HardDrive className="w-4 h-4 sm:w-5 sm:h-5" />, count: 98, color: "from-blue-300 to-slate-500", bgColor: "bg-blue-500/10" },
              { id: 5, name: "Mainboard", description: "Bo mạch chủ", icon: <Cpu className="w-4 h-4 sm:w-5 sm:h-5" />, count: 76, color: "from-slate-300 to-blue-400", bgColor: "bg-slate-500/10" },
              { id: 6, name: "Monitor", description: "Màn hình", icon: <Monitor className="w-4 h-4 sm:w-5 sm:h-5" />, count: 156, color: "from-blue-500 to-slate-600", bgColor: "bg-blue-500/10" },
            ];
