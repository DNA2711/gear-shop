      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-gradient-to-b from-white via-gray-50 to-gray-100 shadow-2xl z-[70] transform transition-all translate-x-0 duration-500 ease-out border-l-2 border-gradient-to-b from-blue-400 to-cyan-400">
        ...
        <div className="flex flex-col h-full bg-gradient-to-b from-gray-50 to-white">
          {cart.items.length === 0 ? (
            ...
            <>
              {/* Cart Items with modern styling */}
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="space-y-4">
                  {cart.items.map((item) => (
                    <div key={item.product_id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200">
                      <CartItem item={item} />
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-gradient-to-r from-gray-50 via-blue-50 to-gray-50 border-t border-blue-100 p-6 space-y-5">
                {/* Clear Cart Button */}
                {cart.items.length > 0 && (
                  <button
                    onClick={handleClearCart}
                    disabled={isLoading}
                    className="flex items-center gap-2 text-red-500 hover:text-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                  >
                ...
                {/* Total Summary with modern card */}
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-blue-100">
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600">Tổng số lượng:</span>
                      <span className="font-medium text-gray-800">{cart.total_items} sản phẩm</span>
                    </div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full py-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-2xl hover:from-blue-700 hover:to-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 font-semibold tracking-wide shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
                  >
                    {isLoading ? (
                  ...
                  <button
                    onClick={onClose}
                    className="w-full py-3 border-2 border-gray-200 text-gray-700 rounded-2xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 font-medium"
                  >
                    Tiếp tục mua sắm
                  </button>
                </div>
