...
const VNPayPayment: React.FC<VNPayPaymentProps> = ({
  ...
  const handlePayment = async () => {
    ...
    if (amount < 5000) {
      setError("Số tiền phải từ 5,000 VND trở lên");
      toast.error("Số tiền phải từ 5,000 VND trở lên");
      return;
    }

    if (amount >= 1000000000) {
      setError("Số tiền phải dưới 1 tỷ đồng");
      toast.error("Số tiền phải dưới 1 tỷ đồng");
      return;
    }
    ...
    try {
      ...
      const paymentUrl = data.paymentUrl || data.simulatedPaymentUrl;
