...
const VNPayQRCode: React.FC<VNPayQRCodeProps> = ({
  ...
  const checkPaymentStatus = async () => {
    ...
    if (shouldSucceed) {
      ...
      const updateSuccess = await updateOrderStatus("paid");
      ...
      if (updateSuccess) {
        ...
        console.error("Failed to update order status, payment may not be processed");
      }
    }
  };
    ...
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-lg">
      ...
      <div className="mt-4">
        <button
          onClick={() => {
            setPaymentStatus("success");
            // Order is already created with 'paid' status, no need to update
            // Call onPaymentSuccess callback directly
            if (onPaymentSuccess) {
              onPaymentSuccess();
            }
          }}
