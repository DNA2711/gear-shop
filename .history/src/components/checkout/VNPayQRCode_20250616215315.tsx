...
const VNPayQRCode: React.FC<VNPayQRCodeProps> = ({
  ...
  // Check payment status periodically (in real app, this would check with payment gateway)
  const checkPaymentStatus = async () => {
    try {
      console.log(
        `[VNPayQRCode] Checking payment status for order ${orderId}...`
      );
      // Use the public payment API instead of authenticated orders API
      const response = await fetch(`/api/payment/${orderId}`);

      if (response.ok) {
        const order = await response.json();
        console.log(`[VNPayQRCode] Order status: ${order.status}`);
        if (order.status === "paid") {
          console.log(
            `[VNPayQRCode] Payment detected as PAID! Setting success status...`
          );
          setPaymentStatus("success");
          // Also update the order status in the authenticated system
          await updateOrderStatus("paid");
          console.log(`[VNPayQRCode] Calling onPaymentSuccess callback...`);
          if (onPaymentSuccess) {
            onPaymentSuccess();
          }
        }
      } else {
        console.log(`[VNPayQRCode] Response not ok: ${response.status}`);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (paymentStatus === "pending" && timeLeft > 0) {
      const timer = setTimeout(() => {
        ...
        if (timeLeft % 10 === 0) {
          console.log(
            `[VNPayQRCode] Timer trigger - checking payment status (timeLeft: ${timeLeft})`
          );
          checkPaymentStatus();
        }
      }, 1000);
  ...
  // Add a separate interval to check payment status more frequently
  useEffect(() => {
    if (paymentStatus === "pending") {
      console.log(`[VNPayQRCode] Setting up payment status checker for order ${orderId}`);
      
      const statusChecker = setInterval(() => {
        console.log(`[VNPayQRCode] Interval check - checking payment status...`);
        checkPaymentStatus();
      }, 5000); // Check every 5 seconds

      return () => {
        console.log(`[VNPayQRCode] Cleaning up payment status checker`);
    ...
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-lg">
      ...
      <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
        <p className="text-xs text-yellow-800 text-center mb-2">
          🧪 <strong>Test Mode:</strong> Để test thanh toán nhanh
        </p>
        <button
          onClick={async () => {
            try {
              console.log(
                `[VNPayQRCode] Test button clicked - completing payment for order ${orderId}`
              );
              ...
              if (response.ok) {
                ...
                // Force check payment status immediately
                setTimeout(() => {
                  console.log(
                    "[VNPayQRCode] Force checking payment status after test button..."
                  );
                  checkPaymentStatus();
                }, 1000);
              }
