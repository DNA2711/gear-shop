    ...
    const getNgrokUrl = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.details || "Failed to get payment URL");
        }
        const data = await response.json();
        if (data.url) {
          setPaymentUrl(`${data.url}/payment/${orderId}`);
        } else {
        ...
        setError(
          error instanceof Error ? error.message : "Failed to get payment URL"
        );
        // Fallback to localhost if can't get ngrok URL
        setPaymentUrl(`${window.location.origin}/payment/${orderId}`);
      }
    };
    getNgrokUrl();
  }, [orderId]);
