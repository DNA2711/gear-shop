                ...
                {activeTab === "notifications" && (
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      {[
                        { key: "email", label: "Thông báo email", icon: Mail, description: "Nhận thông báo qua email", color: "from-blue-500 to-cyan-500", bgColor: "from-blue-50/50 to-cyan-50/50" },
                        { key: "push", label: "Thông báo đẩy", icon: Smartphone, description: "Thông báo trên thiết bị di động", color: "from-purple-500 to-pink-500", bgColor: "from-purple-50/50 to-pink-50/50" },
                        { key: "orderUpdates", label: "Cập nhật đơn hàng", icon: Bell, description: "Thông báo về trạng thái đơn hàng", color: "from-emerald-500 to-teal-500", bgColor: "from-emerald-50/50 to-teal-50/50" },
                        { key: "promotions", label: "Khuyến mãi", icon: Bell, description: "Thông báo về ưu đãi và khuyến mãi", color: "from-orange-500 to-red-500", bgColor: "from-orange-50/50 to-red-50/50" },
                        { key: "newProducts", label: "Sản phẩm mới", icon: Bell, description: "Thông báo về sản phẩm mới", color: "from-violet-500 to-purple-500", bgColor: "from-violet-50/50 to-purple-50/50" },
                      ].map((notif) => {
                        const Icon = notif.icon;
                        const isEnabled = settings.notifications[notif.key as keyof typeof settings.notifications];
                        return (
                          <div key={notif.key} className={`group relative bg-gradient-to-br ${notif.bgColor} backdrop-blur-sm rounded-2xl p-6 border border-gray-200/30 hover:border-gray-300/50 transition-all duration-500 hover:shadow-xl hover:-translate-y-1 ${isEnabled ? 'ring-2 ring-blue-500/20' : ''}`}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-4">
                                <div className={`p-4 bg-gradient-to-r ${notif.color} rounded-2xl shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all duration-300`}>
                                  <Icon className="w-6 h-6 text-white" />
                                </div>
                                <div>
                                  <h3 className="text-xl font-bold text-gray-900 mb-1">{notif.label}</h3>
