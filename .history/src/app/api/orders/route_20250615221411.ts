    ...
    try {
      ...
      for (const item of items) {
        ...
        if (isNaN(price) || price < 0) {
          throw new Error(`Invalid price for product ${item.product_id}: ${price}`);
        }
        
        if (price > 99999999.99) {
          throw new Error(`Price too large for product ${item.product_id}: ${price}`);
        }
        
        totalAmount += price * item.quantity;

        console.log(`Inserting order_item: orderId=${orderId}, productId=${item.product_id}, quantity=${item.quantity}, price=${price}`);
