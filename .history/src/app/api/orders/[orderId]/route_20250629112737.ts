  ...
  try {
    ...
    if (token) {
      try {
        ...
        console.log("Invalid token or no auth, checking if order is accessible");
      }
    }
    ...
    if (userId && orderRows[0].user_id !== userId) {
      console.log("Access denied: User doesn't own order", { userId, orderUserId: orderRows[0].user_id });
      return NextResponse.json(
        { error: "Access denied" },
        { status: 403 }
      );
