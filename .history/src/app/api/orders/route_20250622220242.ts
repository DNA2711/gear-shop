...
export async function POST(req: Request) {
  try {
    ...
    const { shipping_address, phone_number, payment_method = "cod", items } = body;

    // Start transaction using the database helper
    const connection = await db.beginTransaction();
    try {
      // Create order with 'paid' status (since we removed pending logic)
      const orderId = await db.insert(
        `INSERT INTO orders (user_id, total_amount, shipping_address, phone_number, status)
         VALUES (?, ?, ?, ?, 'paid')`,
...
export async function GET(req: Request) {
  try {
    ...
    // First, get all orders for the user (exclude pending orders)
