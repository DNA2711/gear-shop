  ...
  try {
    ...
    const currentRevenue = await db.queryFirst(
      `SELECT SUM(total_amount) as total
       FROM orders
       WHERE created_at >= ? AND created_at <= ? AND status NOT IN ('cancelled', 'pending')`,
    ...
    // Get recent orders (last 5) - exclude pending and cancelled orders
    ...
    // Get sales data for chart (last 12 months) - exclude pending and cancelled orders
