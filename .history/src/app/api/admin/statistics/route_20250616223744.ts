...
export async function GET(request: Request) {
  try {
    ...
    const topProducts = await db.query(
      `SELECT p.product_id, p.name, p.image_url, SUM(oi.quantity) as total_sold, SUM(oi.quantity * oi.price) as total_revenue
       FROM order_items oi
    ...
    const categoryDistribution = await db.query(
      `SELECT c.name, COUNT(DISTINCT oi.order_id) as order_count
       FROM order_items oi
       JOIN products p ON oi.product_id = p.product_id
