...
export async function GET(request: Request) {
  try {
    // Kiểm tra môi trường và trả về URL phù hợp
    const host = request.headers.get("host");

    // Nếu đang chạy trên Vercel (có VERCEL_URL hoặc host chứa vercel.app)
    if (process.env.VERCEL_URL || (host && host.includes("vercel.app"))) {
    ...
    // Development mode - thử ngrok trước, fallback về localhost
