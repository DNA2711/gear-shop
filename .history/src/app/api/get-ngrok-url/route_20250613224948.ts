...
export async function GET() {
  try {
    ...
    try {
      ...
      const ngrokPath = path.join(process.cwd(), 'node_modules', 'ngrok', 'bin', 'ngrok.exe');
      ...
      if (process.env.NGROK_AUTH_TOKEN) {
        await execAsync(`"${ngrokPath}" authtoken ${process.env.NGROK_AUTH_TOKEN}`);
      }

      // Start ngrok tunnel
      const { stdout } = await execAsync(`"${ngrokPath}" http 3000 --log=stdout`);
