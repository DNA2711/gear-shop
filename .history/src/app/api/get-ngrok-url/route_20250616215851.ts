...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      throw new Error(
        "Failed to get ngrok URL after multiple retries. Make sure ngrok is installed and running, or try starting it manually with: ngrok http " +
          currentPort
      );
    } catch (error) {
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            "Try manually starting ngrok: 'ngrok http 3001' or check if NGROK_AUTH_TOKEN is set",
        },
        { status: 500 }
      );
    }
