...
export async function GET(request: Request) {
  try {
    ...
    const forceNgrok = url.searchParams.get("force") === "ngrok";

    // Check if we're running on Vercel production
    if (process.env.VERCEL_URL && !forceNgrok) {
      ...
      return NextResponse.json({
        ...
        source: "vercel",
        note: "Add ?force=ngrok to use ngrok URL instead",
      });
    }

    // Force ngrok mode hoặc development mode
    if (forceNgrok || process.env.NODE_ENV === "development") {
      console.log("Development mode or force ngrok - attempting to use ngrok");
