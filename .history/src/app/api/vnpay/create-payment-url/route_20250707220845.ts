...
const IS_DEMO_MODE = VNPAY_TMN_CODE === "DEMO" || VNPAY_HASH_SECRET === "DEMO_SECRET";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (IS_DEMO_MODE) {
      // Xử lý baseUrl một cách rõ ràng với nhiều fallback
      let baseUrl;
      ...
      if (process.env.NEXT_PUBLIC_BASE_URL) {
        ...
        // Fallback: lấy từ request headers
      ...
      const mockPaymentUrl = `${baseUrl}/vnpay/checkout?orderId=${orderId}&amount=${amount}&orderInfo=${encodeURIComponent(orderInfo)}`;
      
      console.log("Using mock VNPay (DEMO mode):", {
        url: mockPaymentUrl,
        environment: process.env.NODE_ENV,
        baseUrl,
