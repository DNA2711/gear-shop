...
export async function POST(request: NextRequest) {
  try {
    ...
    // Chuyển đổi số tiền sang format của VNPAY (nhân 100)
    // VD: 1,000,000 VND (1 triệu) -> 100,000,000 (VNPAY format)
    const vnpAmount = amountNum * 100;
    console.log("Original amount:", amountNum, "VND");
    console.log("VNPay amount:", vnpAmount, "(after multiplying by 100)");

    // Kiểm tra giới hạn số tiền theo yêu cầu của VNPAY (số tiền sau khi nhân 100)
    if (vnpAmount < 500000 || vnpAmount >= 100000000000) {
    ...
    // Log chi tiết số tiền và các tham số
    console.log("Payment details:", {
      originalAmount: amountNum,
      vnpAmount: vnpAmount,
      formattedAmount: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(amountNum)
    });
    console.log("Full VNPay parameters:", vnpParams);
