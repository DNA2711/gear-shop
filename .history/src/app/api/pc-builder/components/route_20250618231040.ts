...
export async function GET(request: NextRequest) {
  try {
    ...
    // Create separate parameter array for products query
    const productsParams = [...queryParams, limit, offset];
    
    // Debug logging
    console.log("Products Query:", productsQuery);
    console.log("Query Params:", productsParams);
    console.log("Expected placeholders:", (productsQuery.match(/\?/g) || []).length);
    console.log("Actual params count:", productsParams.length);
    
    const products = await dbHelpers.query(productsQuery, productsParams);
