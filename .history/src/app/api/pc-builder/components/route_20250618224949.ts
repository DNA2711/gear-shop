...
export async function GET(request: NextRequest) {
  try {
    ...
    const filter: ComponentFilter = {
      ...
      power_rating: searchParams.get('power_rating') ? parseInt(searchParams.get('power_rating')!) : undefined,
    ...
    // Search filter
    if (filter.search) {
      whereClauses.push('(p.product_name LIKE ? OR p.product_code LIKE ? OR b.brand_name LIKE ?)');
    ...
    if (filter.power_rating) {
      ...
      queryParams.push(`%${filter.power_rating}W%`, `%${filter.power_rating}W%`);
    }
    ...
    const components: PCComponent[] = products.map((product: any) => {
      ...
      const categoryCode = product.parent_category_code || product.category_code;
