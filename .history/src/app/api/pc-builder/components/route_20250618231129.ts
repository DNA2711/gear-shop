...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build count query manually
    let finalCountQuery = countQuery;
    let countParamIndex = 0;

    finalCountQuery = finalCountQuery.replace(/\?/g, () => {
      const param = queryParams[countParamIndex++];
    ...
    const countResult = await db.queryRaw(finalCountQuery);
    ...
    // Build query manually to avoid prepared statement issues
    let finalQuery = productsQuery;
    let paramIndex = 0;

    // Replace each ? with actual values
    finalQuery = finalQuery.replace(/\?/g, () => {
      const param = [...queryParams, limit, offset][paramIndex++];
      if (typeof param === "string") {
    ...
    const products = await db.queryRaw(finalQuery);
