...
export async function GET(request: NextRequest) {
  try {
    ...
    // Debug logging
    console.log("Final Query:", notifQuery);
    console.log("Query Params:", queryParams);
    console.log("Values - userId:", userId, "safeLimit:", safeLimit, "safeOffset:", safeOffset);
    
    let notifications;
    try {
      notifications = await db.query(notifQuery, queryParams);
    } catch (queryError) {
      console.error("Query failed, trying fallback:", queryError);
      // Fallback to simple query without pagination
      const fallbackQuery = "SELECT notification_id, title, message, type, category, is_read, created_at, data FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 5";
      notifications = await db.query(fallbackQuery, [userId]);
    }
