  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const fallbackQuery = `SELECT notification_id, title, message, type, category, is_read, created_at, data FROM notifications WHERE user_id = ${userId} ORDER BY created_at DESC LIMIT ${safeLimit}`;
