  ...
  try {
    ...
    if (filters.search) {
      whereConditions.push(
        "(p.product_name LIKE ? OR p.product_code LIKE ? OR p.description LIKE ? OR b.brand_name LIKE ? OR c.category_name LIKE ?)"
      );
      queryParams.push(
        `%${filters.search}%`,
        `%${filters.search}%`,
    ...
    return NextResponse.json(
      createErrorResponse(status, ErrorMessages.PRODUCT.FETCH_ERROR),
      { status }
    );
  }
}
  ...
  try {
    ...
    if (!body.product_name || !body.product_code || !body.price) {
      return NextResponse.json(
        createErrorResponse(
          400,
          "Tên sản phẩm, mã sản phẩm và giá là bắt buộc"
        ),
        { status: 400 }
      );
    }
    ...
    if (body.price <= 0) {
      return NextResponse.json(
        createErrorResponse(400, ErrorMessages.VALIDATION.PRICE_INVALID),
        { status: 400 }
      );
    }
