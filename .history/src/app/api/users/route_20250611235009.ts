...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!Number.isInteger(limit) || !Number.isInteger(offset) || limit <= 0 || offset < 0) {
      throw new Error(`Invalid pagination params: limit=${limit}, offset=${offset}`);
    }
    ...
    // Workaround: Build query with actual values instead of prepared statement
    // để tránh lỗi "Incorrect arguments to mysqld_stmt_execute"
    let query = `
      SELECT 
        user_id,
        full_name,
    ...
    console.log("DEBUG: Final query with values:", query);
    console.log("DEBUG: Where params:", params);
