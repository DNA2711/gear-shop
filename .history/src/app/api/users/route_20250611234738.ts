...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("DEBUG: Page type:", typeof page, "Limit type:", typeof limit);
    ...
    console.log("DEBUG: Main query:", query);
        ...
        // Build final params array - đảm bảo thứ tự đúng
    const finalParams = [...params, limit, offset];
    console.log("DEBUG: Query params array:", finalParams);
    console.log("DEBUG: Query params types:", finalParams.map(p => typeof p));
    console.log("DEBUG: Query params length:", finalParams.length);
    
    // Count số lượng ? trong query để debug
    const placeholderCount = (query.match(/\?/g) || []).length;
    console.log("DEBUG: Placeholder count in query:", placeholderCount);
    console.log("DEBUG: Params count:", finalParams.length);
    
    if (placeholderCount !== finalParams.length) {
      console.error("ERROR: Placeholder count mismatch!");
      console.error("Query:", query);
      console.error("Expected placeholders:", placeholderCount);
      console.error("Actual params:", finalParams.length);
      throw new Error(`Parameter count mismatch: expected ${placeholderCount}, got ${finalParams.length}`);
    }

    const users = await db.query(query, finalParams);
    ...
    const transformedUsers = users.map((user: any) => ({
      ...
      totalOrders: 0, // Bỏ orders, luôn là 0
      totalSpent: 0, // Bỏ orders, luôn là 0
