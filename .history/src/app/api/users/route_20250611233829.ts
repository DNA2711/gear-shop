...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("DEBUG: Orders table doesn't exist or is empty:", error.message);
      hasOrdersTable = false;
    }

    // Get users with pagination - only use existing columns
    let query;
    if (hasOrdersTable) {
      query = `
        SELECT 
...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertParams = [name, email, hashedPassword, phone || null, address || null, role.toUpperCase()];
