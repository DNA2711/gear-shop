...
export async function PUT(
  ...
  try {
    ...
    // Skip status update since no is_active column
    // if (status !== undefined) {
    ...
    // Update without updated_at since column may not exist
    await db.update(`
      UPDATE users 
    ...
    const updatedUser = await db.queryFirst(updateQuery, [userId]);
...
export async function DELETE(
  ...
  try {
    ...
    let hasOrders = false;
    try {
      const orderCheck = await db.queryFirst("SELECT COUNT(*) as count FROM orders WHERE user_id = ?", [userId]);
