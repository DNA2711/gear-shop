...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    console.log("🔍 Loading selected items from localStorage...");
    ...
    console.log("🔍 savedSelectedItems type:", typeof savedSelectedItems);
    console.log(
      "🔍 savedSelectedItems length:",
      savedSelectedItems ? savedSelectedItems.length : 0
    );

    if (savedSelectedItems) {
      try {
        const parsedItems = JSON.parse(savedSelectedItems);
        console.log("🔍 Found selected items:", parsedItems);
        ...
        setTimeout(() => {
          ...
          console.log("🔍 Items loaded successfully and localStorage cleaned");
        }, 100);
      } catch (error) {
        console.error("🔍 Error parsing selected items:", error);
        setItemsLoaded(true);
        // Tạm thời không redirect, chỉ hiển thị lỗi
        alert("Có lỗi xảy ra khi tải dữ liệu. Vui lòng quay lại giỏ hàng.");
      }
    } else {
      console.log("🔍 No selected items found, showing message");
      console.log("🔍 All localStorage content:", localStorage);
      setItemsLoaded(true);
      // Tạm thời không redirect, chỉ hiển thị thông báo
      alert(
        "Không tìm thấy sản phẩm được chọn. Vui lòng quay lại giỏ hàng và chọn sản phẩm."
      );
    }
  }, [router, itemsLoaded]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="max-w-md w-full relative">
            ...
            <button
              ...
              className="absolute -top-4 -right-4 bg-white text-gray-600 hover:text-gray-800 rounded-full w-8 h-8 flex items-center justify-center shadow-lg z-10"
            >
              ✕
            </button>
            <VNPayQRCode
