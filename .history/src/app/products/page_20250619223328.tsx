...
export default function ProductsPage() {
  ...
  const handleCategoryChange = (categoryId: string | null) => {
    ...
    const newUrl = newParams.toString() ? `/products?${newParams.toString()}` : '/products';
    router.push(newUrl);
  ...
  const currentCategoryName = selectedCategory && categories.length > 0
    ? categories.find((cat) => cat.id && cat.id.toString() === selectedCategory)?.name
    : null;
