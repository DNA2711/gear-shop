...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    fetchProducts(1, "", selectedCategory, selectedBrand);
  }, [selectedCategory, selectedBrand, mounted]);

  // Only fetch when page changes (not on initial mount)
  useEffect(() => {
    if (!mounted || !isMounted.current || page <= 1) return;

    fetchProducts(page, searchTerm, selectedCategory, selectedBrand);
  }, [page, mounted]);
  ...
  const handleCategoryChange = (categoryId: string) => {
    ...
    // Update URL
    updateURL(newSelectedCategory, selectedBrand);

    // Refetch products
    isMounted.current = true; // Allow re-fetch
    fetchProducts(1, "", newSelectedCategory, selectedBrand);
  };
  ...
  const handleBrandChange = (brandId: string) => {
    ...
    // Update URL
    updateURL(selectedCategory, newSelectedBrand);

    // Refetch products
    isMounted.current = true; // Allow re-fetch
    fetchProducts(1, "", selectedCategory, newSelectedBrand);
  };

  const updateURL = (categoryId: string | null, brandId: string | null) => {
    try {
          ...
          <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full p-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white appearance-none"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {brands.map((brand) => (
                      <option key={brand.brand_id} value={brand.brand_id.toString()}>
                        {brand.brand_name}
                      </option>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                  ...
                  >
                    <div className="flex flex-col items-center justify-center h-12">
                      ...
                      <span className="text-xs text-gray-600 mt-1">Tất cả</span>
                    </div>
                  </button>
                  
                  {brands.slice(0, 19).map((brand) => (
                    <button
                      key={brand.brand_id}
                    ...
                    >
                      ...
                      <div className={`flex flex-col items-center justify-center h-8 ${brand.logo_code ? 'hidden' : ''}`}>
                        <span className="text-lg font-bold text-blue-600">
                          {brand.brand_name.charAt(0)}
                        </span>
                      </div>
                      <span className="text-xs text-gray-600 mt-1 truncate block">
                        {brand.brand_name}
                      </span>
                    </button>
                  ))}
            ...
            {(selectedCategory || selectedBrand || searchTerm) && (
              <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-gray-200">
                <span className="text-sm font-medium text-gray-700">Bộ lọc hiện tại:</span>
                ...
                <p className="text-sm text-gray-600">
                  {searchLoading ? (
                    ...
                    <>
                      ...
                      {selectedCategory && ` trong ${categories.find(c => c.id?.toString() === selectedCategory)?.name}`}
                      {selectedBrand && ` của ${brands.find(b => b.brand_id.toString() === selectedBrand)?.brand_name}`}
                    </>
                  )}
                </p>
              </div>
          ...
          <div className="text-center py-20 bg-white rounded-lg shadow-sm">
            ...
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              {searchTerm && selectedCategory
                ? `Không có sản phẩm nào phù hợp với từ khóa "${searchTerm}" trong danh mục ${currentCategoryName}.`
                : searchTerm
                ? `Không có sản phẩm nào phù hợp với từ khóa "${searchTerm}".`
                : selectedCategory
                ? `Danh mục ${currentCategoryName} hiện tại chưa có sản phẩm nào.`
