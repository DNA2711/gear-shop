              ...
              <p className="text-sm text-gray-600">
                {searchLoading ? (
                  ...
                  <>
                    {searchTerm && selectedCategory && (
                      <>
                        Kết quả tìm kiếm cho <strong>"{searchTerm}"</strong> trong danh mục <strong>{currentCategoryName}</strong>: {products.length} sản phẩm
                      </>
                    )}
                    {searchTerm && !selectedCategory && (
                      <>
                        Kết quả tìm kiếm cho <strong>"{searchTerm}"</strong>: {products.length} sản phẩm
                      </>
                    )}
                    {!searchTerm && selectedCategory && (
                      <>
                        Hiển thị {products.length} sản phẩm trong danh mục <strong>{currentCategoryName}</strong>
                      </>
                    )}
                    {products.length === 0 && (
                      <span className="ml-2">
