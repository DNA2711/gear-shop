...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (minPriceParam || maxPriceParam) {
        ...
        const max = maxPriceParam ? parseInt(maxPriceParam) : 50000000;
  ...
  const fetchProducts = async (
    ...
    priceRangeValue: [number, number] = [0, 50000000]
  ) => {
    try {
      if (currentPage === 1) {
        setLoading(true);
      ...
      if (min > 0 || max < 50000000) {
        if (min > 0) params.append("min_price", min.toString());
        if (max < 50000000) params.append("max_price", max.toString());
      }
  ...
  const handlePriceRangeChange = (newPriceRange: [number, number]) => {
    ...
    // Update URL
    updateURL(selectedCategory, selectedBrand, newPriceRange);

    // Refetch products with debounce
    isMounted.current = true;
    fetchProducts(
      1,
      searchTerm,
  ...
  const resetPriceRange = () => {
    const defaultRange: [number, number] = [0, 50000000];
    setPriceRange(defaultRange);
    setMinPrice(0);
    setMaxPrice(50000000);
    setPage(1);

    updateURL(selectedCategory, selectedBrand, defaultRange);
    isMounted.current = true;
    fetchProducts(1, searchTerm, selectedCategory, selectedBrand, defaultRange);
  };
  ...
  const updateURL = (
    ...
    try {
      ...
      const [min, max] = priceRangeValue;
      if (min > 0 || max < 50000000) {
        if (min > 0) urlParams.set("min_price", min.toString());
        if (max < 50000000) urlParams.set("max_price", max.toString());
      }
                      ...
                      <div className="flex flex-col items-center justify-center h-16">
                        {brand.logo_code ? (
                          <img
                            ...
                            className="w-full h-10 object-contain transition-transform group-hover:scale-110"
                            onError={(e) => {
                        ...
                        <span
                          className={`text-xs font-medium mt-2 truncate block w-full text-center ${
                            selectedBrand === brand.brand_id.toString()
                              ? "text-blue-700"
                              : "text-gray-600"
                          }`}
              ...
              <div className="px-3 py-4 bg-gray-50 rounded-xl">
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="text-center">
                    ...
                    <div className="text-sm font-semibold text-gray-900">
                      {new Intl.NumberFormat("vi-VN").format(priceRange[0])} VNĐ
                    </div>
                  </div>
                  <div className="text-center">
                ...
                <div className="relative">
                  ...
                  <div className="h-2 bg-gray-300 rounded-full relative">
                    {/* Active Track */}
                    <div
                      className="absolute h-2 bg-gradient-to-r from-green-500 to-green-600 rounded-full"
                      style={{
                        left: `${(priceRange[0] / 50000000) * 100}%`,
                        width: `${
                          ((priceRange[1] - priceRange[0]) / 50000000) * 100
                        }%`,
                      }}
                    />
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-gray-200">
                ...
                {(priceRange[0] > 0 || priceRange[1] < 50000000) && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-green-100 text-green-800">
                    💰 {new Intl.NumberFormat("vi-VN", {style: "compact", notation: "compact"}).format(priceRange[0])} - {priceRange[1] >= 50000000 ? "50+ triệu" : new Intl.NumberFormat("vi-VN", {style: "compact", notation: "compact"}).format(priceRange[1])}
                    <button
                      onClick={resetPriceRange}
                ...
                <button
                  onClick={() => {
                    ...
                    setMinPrice(0);
                    setMaxPrice(50000000);
                    updateURL(null, null, [0, 50000000]);
                    fetchProducts(1, "", null, null, [0, 50000000]);
                  }}
