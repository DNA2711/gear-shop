...
export default function CartPage() {
  ...
  const calculateSelectedTotals = () => {
    const selectedCartItems = cart.items.filter(item => selectedItems.has(item.product_id));
    const total_items = selectedCartItems.reduce((sum, item) => sum + item.quantity, 0);
    const total_price = selectedCartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    return { total_items, total_price, selectedCartItems };
  };

  const { total_items: selectedTotalItems, total_price: selectedTotalPrice } = calculateSelectedTotals();
  const isAllSelected = selectedItems.size === cart.items.length && cart.items.length > 0;
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        ...
                        >
                          <div className="w-6 h-6 bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg border-2 border-slate-300 peer-checked:border-emerald-500 peer-checked:from-emerald-400 peer-checked:to-emerald-600 transition-all duration-300 ease-out group-hover:scale-105 shadow-sm peer-checked:shadow-emerald-200 peer-checked:shadow-lg flex items-center justify-center">
                            <svg 
                              className="w-4 h-4 text-white transform scale-0 peer-checked:scale-100 transition-transform duration-300 ease-out drop-shadow-sm" 
                              fill="none" 
                              stroke="currentColor" 
                              strokeWidth="2.5" 
                              viewBox="0 0 24 24"
