  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        responseData = { success: false, message: "Lỗi khi xử lý phản hồi từ server" };
      }
      ...
      if (isSuccess) {
        setIsSubmitted(true);
        setMessage("Đã gửi email hướng dẫn reset mật khẩu!");
        
        if (process.env.NODE_ENV === "development" && responseData.resetUrl) {
          showToast("Development Mode: Check console for reset link", "info");
        }
      } else {
        const errorMessage = responseData.message || "Có lỗi xảy ra khi gửi email";
