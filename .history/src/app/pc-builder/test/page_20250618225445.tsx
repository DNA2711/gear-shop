...
export default function PCBuilderTestPage() {
  ...
  const testAPIs = async () => {
    ...
    try {
      ...
      const cpuResponse = await get(
        "/api/pc-builder/components?category_code=cpu&limit=5"
      );
      console.log("CPU Response:", cpuResponse);

      const vgaResponse = await get(
        "/api/pc-builder/components?category_code=vga&limit=5"
      );
      console.log("VGA Response:", vgaResponse);

      const ramResponse = await get(
        "/api/pc-builder/components?category_code=ram&limit=5"
      );
