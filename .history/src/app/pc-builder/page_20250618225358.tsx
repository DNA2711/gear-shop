...
function PCBuilderContent() {
  ...
  const renderComponentSlot = (componentType: string) => {
    ...
    const renderSingleComponent = (comp: PCComponent, index?: number) => (
      <div key={index || 0} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-4">
        <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
          {comp.primary_image ? (
            <img
              src={`data:image/jpeg;base64,${comp.primary_image}`}
        ...
        <div className="flex-1 min-w-0">
          <div className="font-medium text-sm truncate">{comp.product_name}</div>
          {comp.brand_name && (
            <div className="text-xs text-gray-500">{comp.brand_name}</div>
          )}
          <div className="text-blue-600 font-semibold">{formatPrice(comp.price)}</div>
        ...
        <div className="space-y-3">
          {component ? (
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              <span className="text-4xl opacity-30 block mb-2">{category.icon}</span>
              <p className="text-gray-500 mb-3">Chưa chọn {category.name.toLowerCase()}</p>
              <button
    ...
    <div className="min-h-screen bg-gray-100">
                ...
                <div className="space-y-3">
                  ...
                  <div className="border-t pt-3">
                    <div className="text-sm text-gray-600 space-y-1">
                      <div>Linh kiện đã chọn: {Object.values(currentBuild.components).filter(c => c).length}</div>
                      <div>Bắt buộc: {Object.entries(COMPONENT_CATEGORIES).filter(([_, cat]) => cat.required).length}</div>
                    </div>
                  </div>
                </div>
                ...
                {compatibility_status.is_compatible ? (
                  ...
                  <div className="space-y-2">
                    {compatibility_status.errors.map((error, index) => (
                      <div key={index} className="text-red-600 text-sm flex items-start gap-2">
                ...
                {compatibility_status.warnings.length > 0 && (
                  <div className="mt-4 space-y-2">
                    ...
                    {compatibility_status.warnings.map((warning, index) => (
                      <div key={index} className="text-yellow-600 text-sm flex items-start gap-2">
                        <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" />
      ...
      {selectedComponentType && (
        <ComponentSelector
          ...
          onSelect={(component) => addComponent(selectedComponentType, component)}
