...
export default function EditProductPage() {
  ...
  const reloadImages = async () => {
    try {
      ...
      if (productData.success) {
        const sortedImages = (productData.data.images || []).sort(
          (a: any, b: any) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (productData.success) {
          ...
          console.log("DEBUG - Product details from API:", productDetails);
          console.log(
            "DEBUG - is_active value:",
            productDetails.is_active,
            "type:",
            typeof productDetails.is_active
          );
          ...
          setFormData({
            ...
            is_active: Boolean(productDetails.is_active),
          });
  ...
  const handleAddImageUrl = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Lỗi: ${error.error || "Không thể tải ảnh từ URL"}`);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      console.log("DEBUG - Saving formData:", formData);
      console.log(
        "DEBUG - is_active in formData:",
        formData.is_active,
        "type:",
      ...
      // Save specifications
      console.log("Saving specifications:", specifications);
      ...
      console.log("Delete specifications response:", deleteResult);

      if (!deleteResponse.ok) {
        console.error("Failed to delete specifications:", deleteResult);
        throw new Error(
          deleteResult.error || "Failed to delete existing specifications"
        );
      }
      ...
      for (let i = 0; i < specifications.length; i++) {
        ...
        console.log(`Adding specification ${i + 1}:`, spec);
        ...
        console.log(`Specification ${i + 1} result:`, specResult);

        if (!specResponse.ok) {
          console.error(`Failed to add specification ${i + 1}:`, specResult);
          throw new Error(
            specResult.error || `Failed to add specification: ${spec.spec_name}`
          );
        }
      }
      ...
      for (let i = 0; i < existingImages.length; i++) {
        ...
        if (img.is_primary !== shouldBePrimary || img.display_order !== i) {
          console.log(`Updating existing image ${img.image_id}:`, {
            isPrimary: shouldBePrimary,
            displayOrder: i,
          });

          const response = await fetch(
            `/api/products/${productId}/images/${img.image_id}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
          ...
          const result = await response.json();
          console.log(`Update existing image ${img.image_id} result:`, result);

          if (!response.ok) {
            throw new Error(`Failed to update existing image: ${result.error}`);
          }
        }
      }

      // Reload images to show updated data
      await reloadImages();
      ...
      // Optionally reload product data without full page reload
      const updatedProduct = await fetch(`/api/products/${productId}`).then(
        (res) => res.json()
      );
      if (updatedProduct.success) {
        setProduct(updatedProduct.data);
        setSpecifications(updatedProduct.data.specifications || []);
      }
    } catch (error) {
      console.error("Error saving product:", error);
      const errorMessage = error instanceof Error ? error.message : "Lỗi khi lưu sản phẩm";
