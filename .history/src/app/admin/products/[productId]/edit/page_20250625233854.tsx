...
export default function EditProductPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      const deleteResponse = await fetch(`/api/products/${productId}/specifications`, {
        method: "DELETE",
      });
      console.log("Delete specifications response:", await deleteResponse.text());
      ...
      for (let i = 0; i < specifications.length; i++) {
        ...
        const specResponse = await fetch(`/api/products/${productId}/specifications`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!specResponse.ok) {
          ...
          throw new Error(specResult.error || `Failed to add specification: ${spec.spec_name}`);
        }
      }
