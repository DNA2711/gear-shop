...
export default function UsersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        alert(result.message || (editingUser ? "Cập nhật thành công" : "Tạo người dùng thành công"));
        closeModal();
        fetchUsers();
      } else {
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end space-x-2">
                          <button
                            ...
                            className={user.status === "inactive" ? "text-green-600 hover:text-green-900" : "text-red-600 hover:text-red-900"}
                            title={user.status === "inactive" ? "Kích hoạt" : "Vô hiệu hóa"}
                          >
                            {user.status === "inactive" ? <CheckCircle className="h-4 w-4" /> : <Ban className="h-4 w-4" />}
                          </button>
                          <button 
