...
export default function StatisticsPage() {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      setCategoryStats(Array.isArray(data.categoryDistribution) ? data.categoryDistribution : []);
              ...
              {safeTopProducts.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={safeTopProducts}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="total_sold" name="Số lượng bán" fill="#3B82F6" />
                    <Bar dataKey="total_revenue" name="Doanh thu" fill="#10B981" />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex items-center justify-center h-full text-gray-500">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      cy="50%"
                      outerRadius={150}
                      label={({ name, order_count }: { name: string; order_count: number }) => 
                        `${name} (${order_count})`
                      }
                    >
