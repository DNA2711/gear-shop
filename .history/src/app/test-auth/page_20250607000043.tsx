...
export default function TestAuthPage() {
  ...
  const addTestResult = (test: string, result: any, status: "success" | "error") => {
  ...
  const testBackendConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addTestResult("Backend Health Check", `Status: ${response.status}, Response: ${data}`, "success");
      } else {
        addTestResult("Backend Health Check", `Status: ${response.status}, Error: ${response.statusText}`, "error");
      }
    } catch (error) {
    ...
    try {
      ...
      addTestResult("Login Endpoint Test", `Status: ${response.status}, Response: ${JSON.stringify(data, null, 2)}`, response.status === 401 ? "success" : "error");
    } catch (error) {
    ...
    try {
      ...
      const data = await response.json();
      addTestResult("Register Endpoint Test", `Status: ${response.status}, Response: ${JSON.stringify(data, null, 2)}`, "success");
    } catch (error) {
  ...
  const testWithRealCredentials = async () => {
    ...
    try {
      ...
      const data = await response.json();
      addTestResult("Real Login Test", `Status: ${response.status}, Response: ${JSON.stringify(data, null, 2)}`, response.ok ? "success" : "error");
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <div className="bg-gray-100 p-4 rounded">
            ...
            <p><strong>Register Endpoint:</strong> {API_CONFIG.ENDPOINTS.AUTH.SIGNUP}</p>
          </div>
        </div>
