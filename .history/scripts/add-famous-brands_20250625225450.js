        ...
        for (const brand of famousBrands) {
            try {
                ...
                const [existing] = await connection.execute(
                    'SELECT brand_id FROM brands WHERE brand_code = ? OR brand_name = ?',
                ...
                const [result] = await connection.execute(
                    `INSERT INTO brands (brand_name, brand_code, description, is_active, created_at, updated_at) 
           VALUES (?, ?, ?, 1, NOW(), NOW())`,
                    [brand.brand_name, brand.brand_code, brand.description]
                );
