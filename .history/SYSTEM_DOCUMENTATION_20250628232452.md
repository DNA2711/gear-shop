...
export const ProductCard = ({
  ...
  const currentCartQuantity = items.find(item => item.product_id === product.product_id)?.quantity || 0;
    ...
    >
      ...
      <LoadingLink href={`/products/${product.product_id}`} className="block">
        ...
        <div className={cn(
          ...
          variant === 'list' ? 'w-24 h-24 flex-shrink-0' : 'aspect-square mb-4'
        )}>
          {product.primary_image ? (
            <img
              src={`data:image/jpeg;base64,${product.primary_image}`}
        ...
        <div className={cn(variant === 'list' && 'ml-4 flex-1')}>
          ...
          <p className={cn(
            ...
            {product.stock_quantity <= 0 ? 'Hết hàng' :
             product.stock_quantity <= 5 ? `Còn ${product.stock_quantity} sản phẩm` : 'Còn hàng'}
          </p>
        </div>
      </LoadingLink>
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addItem = async (newItem: Omit<CartItem, 'quantity'>) => {
    ...
    try {
      // Check current stock
      const stockResponse = await fetch(`/api/products/${newItem.product_id}/stock`);
